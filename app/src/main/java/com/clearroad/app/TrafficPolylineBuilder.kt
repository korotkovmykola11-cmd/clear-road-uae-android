...
internal object TrafficPolylineBuilder {
    /**
     * Builds colored traffic segments for map / Static Maps preview.
     */
    fun build(route: RealRouteDebugData): List<TrafficSegment> {
        ...
        if (route.trafficSpeedIntervals.isNotEmpty()) {
            return mergeConsecutive(
                ...
                TrafficSegmentDebugLog.logBuiltSegments(route, segments)
            }
        }
        ...
        if (route.stepTrafficRecords.isNotEmpty()) {
            ...
            val segments =
                if (LegLevelTrafficApproximation.shouldOverrideUniformFreeSteps(route, stepSegments)) {
                    LegLevelTrafficApproximation.buildUniformRouteSegments(route).orEmpty()
                } else {
                    stepSegments
                }
            return segments.also { built ->
                TrafficSegmentDebugLog.logBuiltSegments(route, built)
            }
        }

        LegLevelTrafficApproximation.buildUniformRouteSegments(route)?.let { segments ->
            return segments.also { built ->
                TrafficSegmentDebugLog.logBuiltSegments(route, built)
            }
        }
